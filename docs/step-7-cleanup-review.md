...
UI follow-up: Delete outdated is the far-right footer action with a muted red style. The status shows `Cleanup HH:mm:ss`; detailed counts appear in a notification that dismisses after 10 seconds. Other below-table action/sync notifications use the same timeout and can still be dismissed manually. Every dropdown uses an inset arrow with space reserved for text, and search uses a vertically centered SVG magnifier. These UI-only adjustments were checked with both TypeScript configurations and a portable build; they require no Convex redeployment.
