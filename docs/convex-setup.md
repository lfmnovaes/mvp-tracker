...
Capture & diagnostics provides Open logs, Clear logs and Copy diagnostics. Logs include operation, safe request ID, elapsed time, failure category and version; diagnostic reports also include sync state. Repeated failures are throttled and unchanged syncs stay quiet. Local logs exclude raw packets, names, clipboard contents and private URLs. See [troubleshooting](troubleshooting.md). Convex's own dashboard logs are separate and may expose function arguments to trusted project operators. [Convex logs](https://docs.convex.dev/dashboard/deployments/logs).
