...
0.1.9.2 and 0.1.9.3 use protocol 4 and database metadata schema 2. When upgrading from before 0.1.9.2, run **npx convex dev --once** from matching source and upgrade all sharing clients. Moving from 0.1.9.2 to 0.1.9.3 requires no backend deployment. The first initialization/sync upgrades schema 1 automatically, discarding retired experimental sightings while preserving kill records. **No Reset is needed.** Reset now rejects pre-reset observation times, rather than freshly re-observed older kills. [Data/field audit](data-model.md) · [Troubleshooting](troubleshooting.md).
