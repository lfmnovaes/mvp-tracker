...
function safeError(error: unknown): SharingError {
  ...
  if (error instanceof Error && /ArgumentValidationError|Object is missing the required field|Object contains extra field/i.test(error.message)) return new SharingError(errors.VERSION!);
...
export class SharingConnection {
  ...
  constructor(root: string, private changed: () => void = () => {}, private fetcher: typeof fetch = globalThis.fetch) {
    ...
    try {
      if (statSync(source).size > 4096) throw new Error();
      const raw = JSON.parse(readFileSync(source, "utf8")); if (raw.schemaVersion !== 1) throw new Error();
      this.config = parseConnection(raw); this.resetStatus();
      if (source === legacy || "groupKey" in raw) {
        const temporary = `${this.file}.tmp`;
        writeFileSync(temporary, JSON.stringify({ schemaVersion: 1, ...this.config }) + "\n", { encoding: "utf8", flush: true }); renameSync(temporary, this.file);
        if (source === legacy) unlinkSync(legacy);
      }
  ...
  configure(raw: Connection): ConnectionStatus {
    ...
    if (next.url === this.config.url && this.status.state !== "error") return this.snapshot();
    try {
