...
export interface CaptureSettings { deviceName: string; manualCharacter: string; coordinates?: boolean; alive?: boolean }
export const captureDefaults = (): CaptureSettings => ({ deviceName: "", manualCharacter: "", coordinates: true, alive: true });
export function parseCaptureSettings(value: unknown): CaptureSettings {
  ...
  if (s.coordinates !== undefined && typeof s.coordinates !== "boolean" || s.alive !== undefined && typeof s.alive !== "boolean") throw new Error("Invalid experimental capture settings.");
  return { deviceName: s.deviceName, manualCharacter: s.manualCharacter.trim(), coordinates: s.coordinates ?? true, alive: s.alive ?? true };
}
export interface CaptureSnapshot {
  ...
  skipped: number;
  unresolved: number;
  experiments?: { bossObjects: number; positions: number; sightings: number };
}
