...
import { COLOR_INTERVALS } from "../shared/colors";
...
type Tab = "General" | "Colors" | "Hotkeys" | "Tracking" | "Sharing" | "Capture" | "Diagnostics";
function App() {
  ...
  return <main>
    ...
    <div class="workspace">
      ...
      {health.state === "unavailable" && <div class="notice warning" role="status">{health.detail}<button onClick={() => { openSettings(); setTab("Capture"); }}>Capture settings</button></div>}
      <section class="tracker-card" aria-label="Boss timers">

        <div class="table-scroll"><table><thead><tr>{([["boss", "Boss"], ["level", "Level"], ["location", "Location"], ["region", "Region"], ["channel", "CH"], ["status", "Status"], ["gathered", "Gathered at"], ["killer", "Killer"]] as [SortField, string][]).map(([field, label]) => <Fragment key={field}>{field === "status" && <th class="coordinates-heading" title="Experimental ground coordinates from world X/Z, not a calibrated game-map grid">X / Y</th>}<th key={field} aria-sort={sort.field === field ? sort.direction === "asc" ? "ascending" : "descending" : "none"}><button disabled={sorting || !connected} onClick={() => void changeSort(field)}>{label}{sort.field === field ? sort.direction === "asc" ? " ↑" : " ↓" : ""}</button></th></Fragment>)}<th class="actions-heading">Actions</th></tr></thead>
          <tbody onFocusCapture={() => setFocusedOrder(current => current ?? rows.map(row => slotKey(row.slot)))} onBlurCapture={e => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setFocusedOrder(null); }}>{displayRows.map(row => <TimerRowView key={slotKey(row.slot)} row={row} now={now} clock24={snapshot?.settings.clock24 ?? true} colorInterval={snapshot?.settings.colorInterval ?? 3} onEdit={slot => setManual({ edit: slot })} onRemove={removeTimer}/>)}</tbody></table>
    ...
    <dialog ref={dialog} onCancel={() => setSettingsOpen(false)} onClose={() => setSettingsOpen(false)} aria-labelledby="settings-title">
      ...
      <div class="settings-body"><nav aria-label="Settings sections">{(["General", "Colors", "Hotkeys", "Tracking", "Sharing", "Capture", "Diagnostics"] as Tab[]).map(t => <button key={t} class={tab === t ? "selected" : ""} onClick={() => { setTab(t); setMessage(""); }}>{t}</button>)}</nav>
        <section class="settings-content">
          {tab === "General" && <><h3>Window & time</h3><label class="check-row"><input type="checkbox" checked={draft.startMinimized} onChange={e => setDraft({ ...draft, startMinimized: e.currentTarget.checked })}/><span>Start hidden in the system tray<small>Applies the next time you open MVP Tracker.</small></span></label>
            ...
            </>}{tab === "Hotkeys" && <><h3>Global shortcuts</h3><p class="help">Click a field and press a shortcut. Backspace clears it. Registered shortcuts consume the key press, including while the game is focused.</p>
          ...
          {tab === "Colors" && <><h3>Gathered at colors</h3><p class="help">Ten fixed colors from green to red. Outdated stays muted red.</p><label class="field">Change color every<select value={draft.colorInterval} onChange={e => setDraft({ ...draft, colorInterval: Number(e.currentTarget.value) })}>{COLOR_INTERVALS.map(minutes => <option key={minutes} value={minutes}>{minutes} min{minutes === 3 ? " (default)" : ""}</option>)}</select></label></>}
          ...
          {tab === "Capture" && <><h3>Capture</h3><p role="status">{health.detail}</p>
            ...
            <h3>Experimental observations</h3><label class="check-row"><input type="checkbox" checked={draft.capture.coordinates ?? true} onChange={e => setDraft({ ...draft, capture: { ...draft.capture, coordinates: e.currentTarget.checked } })}/><span>Capture ground coordinates<small>X/Y uses world X/Z. Missing or unsafe positions show Not located.</small></span></label>
