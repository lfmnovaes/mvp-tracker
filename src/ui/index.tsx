...
type Tab = "General" | "Hotkeys" | "Tracking" | "Sharing" | "Capture" | "Diagnostics";
function App() {
  ...
  return <main>
    ...
    <div class="workspace">
      ...
      <section class="tracker-card" aria-label="Boss timers">
        ...
        <div class="table-scroll"><table><thead><tr>{([["boss", "Boss"], ["level", "Level"], ["location", "Location"], ["region", "Region"], ["channel", "CH"], ["status", "Status"], ["gathered", "Gathered at"], ["killer", "Killer"]] as [SortField, string][]).map(([field, label]) => <Fragment key={field}>{field === "status" && <th class="coordinates-heading" title="Experimental ground coordinates from world X/Z, not a calibrated game-map grid">X / Y</th>}<th key={field} aria-sort={sort.field === field ? sort.direction === "asc" ? "ascending" : "descending" : "none"}><button disabled={sorting || !connected} onClick={() => void changeSort(field)}>{label}{sort.field === field ? sort.direction === "asc" ? " ↑" : " ↓" : ""}</button></th></Fragment>)}<th class="actions-heading">Actions</th></tr></thead>
          <tbody onFocusCapture={() => setFocusedOrder(current => current ?? rows.map(row => slotKey(row.slot)))} onBlurCapture={e => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setFocusedOrder(null); }}>{displayRows.map(row => <TimerRowView key={slotKey(row.slot)} row={row} now={now} clock24={snapshot?.settings.clock24 ?? true} onEdit={slot => setManual({ edit: slot })} onRemove={removeTimer}/>)}</tbody></table>
    ...
    <dialog ref={dialog} onCancel={() => setSettingsOpen(false)} onClose={() => setSettingsOpen(false)} aria-labelledby="settings-title">
      ...
      <div class="settings-body"><nav aria-label="Settings sections">{(["General", "Hotkeys", "Tracking", "Sharing", "Capture", "Diagnostics"] as Tab[]).map(t => <button key={t} class={tab === t ? "selected" : ""} onClick={() => { setTab(t); setMessage(""); }}>{t}</button>)}</nav>
        <section class="settings-content">
          ...
          {tab === "Capture" && <><h3>Capture</h3><p role="status">{health.detail}</p>
            ...
            <h3>Experimental observations</h3><label class="check-row"><input type="checkbox" checked={draft.capture.coordinates ?? true} onChange={e => setDraft({ ...draft, capture: { ...draft.capture, coordinates: e.currentTarget.checked } })}/><span>Capture ground coordinates<small>X/Y uses world X/Z. Missing or unsafe positions show Not located.</small></span></label><label class="check-row"><input type="checkbox" checked={draft.capture.alive ?? true} onChange={e => setDraft({ ...draft, capture: { ...draft.capture, alive: e.currentTarget.checked } })}/><span>Detect living bosses<small>Requires known identity and positive health. Sightings age after one minute and expire after five. Disable if noisy.</small></span></label><h3>Character identity</h3><p>{health.character ? `${health.character} · detected live` : "Waiting for your local character. Move or change maps in the game."}</p>{!health.character && health.cachedCharacter && <p class="help">Last seen: {health.cachedCharacter} (cached for this session; not used as a live identity).</p>}
            ...
            </>}{tab === "Diagnostics" && <><h3>Application health</h3><dl><dt>Version</dt><dd>{VERSION} · Windows 11 x64</dd><dt>Backend</dt><dd>{snapshot ? "Connected" : "Unavailable"}</dd><dt>System tray</dt><dd>{snapshot?.trayReady ? "Ready" : "Unavailable"}</dd><dt>Portable storage</dt><dd>{snapshot?.storageWritable ? "Writable" : "Unavailable / read-only"}</dd><dt>Capture</dt><dd>{captureLabel} · {gameLabel}</dd><dt>Adapter in use</dt><dd>{health.adapter ?? "None"}</dd><dt>Last game packet</dt><dd>{health.lastPacketAt ? formatTimestamp(health.lastPacketAt, snapshot?.settings.clock24) : "None this capture session"}</dd><dt>Retry</dt><dd>{health.retryAt ? formatClock(health.retryAt, snapshot?.settings.clock24) : "Not scheduled"}</dd><dt>Awaiting context</dt><dd>{health.unresolved}</dd><dt>Skipped observations</dt><dd>{health.skipped}</dd><dt>Experimental capture</dt><dd>{health.experiments ? `${health.experiments.bossObjects} boss objects · ${health.experiments.positions} positions · ${health.experiments.sightings} sightings` : "Waiting for packets"}</dd></dl>
