...
function App() {
  ...
  const captureLabel = health.state === "running" ? "Capture active" : health.state === "starting" ? "Capture starting" : "Capture inactive";
  ...
  return <main>
    ...
    <section class="health" aria-label="Capture status" title={health.detail}><span class={`status-dot ${health.state === "running" ? "active" : ""}`}/><strong>{captureLabel}</strong><span class="divider"/><span>{gameLabel}</span><span class="health-right">{snapshot ? "Shell ready" : "Starting…"}</span></section>
    <div class="workspace">
      ...
      <section class="tracker-card" aria-label="Boss timers">
        ...
        {rows.length === 0 && <div class="empty"><div class="empty-icon">◷</div><h2>{noneSelected ? "Nothing selected for tracking" : snapshot?.timers.length ? "No matching timers" : "No boss observations yet"}</h2><p>{noneSelected ? "Choose bosses and regions in Settings to start tracking." : snapshot?.timers.length ? "Try another boss name, region or channel." : "Walk near a gravestone in the game to collect a timer. Capture continues while the window is hidden."}</p><span class="empty-tag">MVP TRACKER · v{VERSION}</span></div>}
      ...
      <div class="settings-body"><nav aria-label="Settings sections">{(["General", "Tracking", "Sharing", "Capture & diagnostics"] as Tab[]).map(t => <button key={t} class={tab === t ? "selected" : ""} onClick={() => { setTab(t); setMessage(""); }}>{t}</button>)}</nav>
        <section class="settings-content">
          ...
          {tab === "Capture & diagnostics" && <><h3>Capture</h3><p role="status">{health.detail}</p>
            ...
            <h3>Essential logs only</h3><p class="help">Logs are stored in the logs folder beside the app. Up to five 2 MiB files are retained for seven days. Packet data, character names, credentials and clipboard contents are never logged.</p><p class="help">Preferences live in data/settings.json beside the app. Extract the entire ZIP into a folder you can write to.</p></>}
        </section>
      </div>
