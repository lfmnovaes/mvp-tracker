...
export function TimerRowView({ row: { slot, boss, status }, now, clock24, colorInterval, onEdit, onRemove }: { row: TimerRow; now: number; clock24: boolean; colorInterval: number; onEdit: (slot: TimerSlot) => void; onRemove: (slot: TimerSlot) => Promise<void> }) {
  ...
  return <><tr ref={element}><td>{boss.name}</td><td>{boss.level}</td><td>{boss.map}</td><td>{slot.region.toUpperCase()}</td><td>{slot.channel}</td>
    ...
    <><dt>Killed at</dt><dd>{formatTimestamp(o.diedAt!, clock24)}</dd><dt>Guaranteed spawn</dt><dd>{formatTimestamp(o.diedAt! + SPAWN_AFTER, clock24)}</dd></>
    <dt>Gathered at</dt><dd>{formatTimestamp(o.gatheredAt, clock24)}</dd><dt>Source</dt><dd>{o.source === "manual" ? "Manual entry" : "Gravestone capture"}</dd>
