...
import { emptySlot, expireSlots, MAX_SLOTS, mergeObservations, parseObservation, parseSlot, slotKey, type Observation, type TimerSlot } from "./timers";
import { EXPIRE_AFTER } from "./time";
...
export function applySync(current: TimerSlot[], previous: SyncCache | undefined, raw: SyncResult, outgoing: Observation[], connectionId: string, selectionKey: string, selection: Selection, now: number): { slots: TimerSlot[]; cache: SyncCache } {
  ...
  const base = expireSlots(current, now).map(s => s.observation && s.observation.diedAt <= dataset.resetAt ? emptySlot(s, true) : s);
