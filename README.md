...
- Add manually works without live capture. Choose a selected boss, region and channel, then enter kill date/time and an optional killer. Bosses sort by level descending, then name. Today/Yesterday buttons use America/Sao_Paulo. The default is 24-hour time; turn it off in General for AM/PM. Existing settings switch to 24-hour once on upgrade, and subsequent preferences are retained. Seconds are optional. Preview shows the interpreted kill and spawn window before Save.
- The trash icon removes one local row, including its saved observation. Capture or a later import/sync can restore it; this does not delete anyone else's shared data.
...
“Shell ready” means the interface is receiving local backend responses. It can also show “Starting…” or “Backend reconnecting…”. Capture and game detection are reported separately. See the [complete header status reference](docs/step-5-follow-up.md#header-status-reference).
