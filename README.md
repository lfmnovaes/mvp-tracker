...
Portable, unsigned Windows 11 x64 boss tracker for Spirit Vale. Version **0.1.9.1**. Local tracking works without Convex.
...
For upgrades, Exit first, replace the application files, and keep your `data/` folder. Kill observations expire after 150 minutes; Alive sightings after five minutes. [Sharing details](docs/convex-setup.md).
...
Experimental X/Y shows ground world X/Z, not a calibrated game-map grid; missing coordinates show **Not located**. Positive boss health sets **Alive** and Gathered at; after one minute it becomes **Last seen alive**. Both experiments can be disabled in Settings → Capture.

Status sorting puts Alive first, then Spawned before Spawn window and Waiting. Updated rows briefly highlight; Gathered at runs from green to red in ten 15-minute freshness bands. Outdated uses muted red and discards its old timestamp.
...
After backend updates—or to retry a failed type-check—run **`npx convex dev --once`**. Ordinary app launches need neither command. Deploy the matching backend before running this URL-only build (sharing protocol 3). For 0.1.9.1, redeploy once and upgrade every sharing client; no Reset is required. Local data and full exports migrate to schema 2 (schema 1 imports still work); keep a data backup before downgrading.
