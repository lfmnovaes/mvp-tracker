...
for (const file of ["src", "native", "scripts", "tests", "docs", "licenses", "convex", "vitest.config.ts", ".env.local.sample", ".gitignore", ".gitattributes", "package.json", "bun.lock", "tsconfig.json", "neutralino.config.json", "README.md", "LICENSE.txt", "THIRD_PARTY_NOTICES.md", "dependency-provenance.md", "plan.md"]) await cp(join(root, file), join(source, file), { recursive: true });
