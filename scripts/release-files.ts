...
export function allowedReleasePath(path: string, source = false): boolean {
...
export async function inventory(root: string, relative = "", source = false): Promise<{ path: string; bytes: number; sha256: string }[]> {
  ...
  for (const name of (await readdir(join(root, relative))).sort()) {
    ...
    if (stat.isDirectory()) result.push(...await inventory(root, path, source));
    else {
      if (!allowedReleasePath(path, source)) throw new Error(`Unexpected package file: ${path}`);
