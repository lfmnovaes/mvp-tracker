...
test("connection origins validate before transmission; IPC shares the same checks", () => {
  ...
  expect(parseConnection({ url: "" })).toEqual({ url: "" });
  expect(parseConnection({ url: config.url + "/" })).toEqual({ url: config.url });
  ...
  expect(parseRequest({ id: "save", method: "sharingSave", input: { ...config, groupKey: "legacy-key" } }).input).toEqual(config);
});
test("URL settings persist separately, legacy key fields are discarded, corrupt originals survive and removal keeps timer data", () => {
  const { c, root } = make(); expect(c.snapshot().configured).toBe(false); c.configure(config);
  expect(JSON.stringify(c.snapshot())).not.toContain("groupKey");
  expect(new SharingConnection(root).credentials()).toEqual(config);
  writeFileSync(join(root, "data", "timers.json"), "timer sentinel");
  c.configure({ url: "" }); expect(readFileSync(join(root, "data", "timers.json"), "utf8")).toBe("timer sentinel");
  expect(JSON.parse(readFileSync(c.file, "utf8"))).toEqual({ schemaVersion: 1, url: "" });
...
test("Test uses only the saved origin and a read-only query with redirects disabled", async () => {
  ...
  expect(body.args).toEqual([{ protocol: 2 }]);
});

test("legacy connection migrates its URL and removes the obsolete key file", () => {
  const { root } = make(); mkdirSync(join(root, "data"));
  const legacy = join(root, "data", "sharing-secrets.json");
  writeFileSync(legacy, JSON.stringify({ schemaVersion: 1, ...config, groupKey: "legacy-secret" }));
  const migrated = new SharingConnection(root);
  expect(migrated.credentials()).toEqual(config); expect(existsSync(legacy)).toBe(false);
  expect(JSON.parse(readFileSync(migrated.file, "utf8"))).toEqual({ schemaVersion: 1, ...config });
});
