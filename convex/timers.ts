...
function authorize(key: string, protocol: number) {
  ...
  if (difference) fail("UNAUTHORIZED");
...
async function expiryMetadata(ctx: MutationCtx, m: Doc<"trackerMeta">) {
  ...
  await ctx.db.patch(m._id, { revision: m.revision, nextExpiry: m.nextExpiry });
}
export const sync = mutation({ args: {
  key: v.string(), protocol: v.number(), datasetId: v.string(), generation: v.number(), sinceRevision: v.union(v.number(), v.null()), requestId: v.string(), sentByCharacter: v.optional(v.string()), observations: v.array(observation),
  ...
  if (args.observations.length && (!sender || sender.length > 80 || /[\u0000-\u001f\u007f]/.test(sender))) fail("CHARACTER_REQUIRED");
  ...
  for (const o of incoming) {
    ...
    const fields = { key, ...parseSlot(o), observation: { ...o, submission: { submittedByCharacter: sender!, serverAcceptedAt: now } }, outdated: false, revision: ++m.revision, expiresAt: o.diedAt + EXPIRE_AFTER };
    if (old) await ctx.db.patch(old._id, fields); else await ctx.db.insert("bossTimers", fields);
  }
  if (m.revision !== before) await expiryMetadata(ctx, m);
...
export const reset = mutation({ args: { key: v.string(), protocol: v.number(), datasetId: v.string(), generation: v.number(), requestId: v.string() }, handler: async (ctx, args): Promise<Dataset> => {
  ...
  await ctx.db.patch(m._id, { generation: m.generation, revision: m.revision, resetAt: m.resetAt, nextExpiry: undefined });
