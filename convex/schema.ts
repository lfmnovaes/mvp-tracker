...
export const observation = v.object({
  ...
  submission: v.optional(v.object({ submittedByCharacter: v.string(), serverAcceptedAt: v.number() })),
});
export default defineSchema({
  trackerMeta: defineTable({ singleton: v.literal("tracker"), schema: v.number(), catalog: v.number(), datasetId: v.string(), generation: v.number(), resetAt: v.number(), revision: v.number(), nextExpiry: v.optional(v.number()) }).index("by_singleton", ["singleton"]),
